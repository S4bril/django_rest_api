...
class TestUserLikeListCreateView(APITestCase):
    ...
    def test_like_yourself(self):
        response = self.client.post(self.url, {"receiver_id": self.user1.id}, format="json")

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("must be different", response.data["detail"])

    def test_create_like_success(self):
        response = self.client.post(self.url, {"receiver_id": self.user2.id}, format="json")

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Like.objects.filter(sender=self.user1, receiver=self.user2).exists())

        notification = Notification.objects.filter(user=self.user2, sender=self.user1, type="like")
    ...
    def test_create_match_on_mutual_like(self):
        ...
        self.assertTrue(Match.objects.filter(first_user__in=[self.user1, self.user2], second_user__in=[self.user1, self.user2]).exists())
        self.assertFalse(Like.objects.filter(sender=self.user2, receiver=self.user1).exists())

        notification1 = Notification.objects.filter(user=self.user1, sender=self.user2, type="match")
        notification2 = Notification.objects.filter(user=self.user2, sender=self.user1, type="match")
